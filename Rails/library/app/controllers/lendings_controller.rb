class LendingsController < ApplicationController
  ...
  # POST /lendings.json
  def create
    @lending = @user.lendings.new(lending_params)

    respond_to do |format|
      if @lending.save
        format.html { redirect_to user_lendings_path(@user.id), notice: 'Lending was successfully created.' }
  ...
  def update
    #params[:lendings][:return_date] = Date.today 
    respond_to do |format|
      if @lending.update(return_date: Date.today)
        format.html { redirect_to user_lendings_path(current_user.id, @lending.id), notice: 'Lending was successfully updated.' }
  ...
  private
    ...
    def lending_params
      params.permit(:lending_date, :return_date, :book_id, :user_id)
    end
end